...
class TrainerController(object):
    def __init__(self, use_gpu, run_id, save_freq, load, train, worker_id, keep_checkpoints, lesson, seed,
        ...
        self.env = FAPSPLMEnvironment(worker_id=self.worker_id)
        ...
        self.brain_name = re.sub('[^0-9a-zA-Z]+', '-', list(self.all_info.keys())[0])
        ...
        try:
            while any([t.get_step <= t.get_max_steps for k, t in self.trainers.items()]) or not self.train_model:
                ...
                if self.env.global_done or any([d != 0 for d in curr_info.local_done]):
                    curr_info = self.env.reset(train_mode=self.train_model)[self.brain_name]
                    for brain_name, trainer in self.trainers.items():
                        trainer.end_episode()
